...
def parse_m8_to_template_hits(
    ...
    for i, row in enumerate(table.itertuples()):
        ...
        import os
        debug_dir = template_cif_folder
        if debug_dir is None:
            logger.warning(f"DEBUG: CHAI_TEMPLATE_CIF_FOLDER is not set, so template_cif_folder is None.")
        else:
            debug_path_to_check = debug_dir / f"{hit_identifier.upper()}.cif.gz"
            logger.info(f"DEBUG: Checking for template at path: {debug_path_to_check}")
            logger.info(f"DEBUG: Path exists? {os.path.exists(debug_path_to_check)}")
            if not os.path.exists(debug_path_to_check):
                try:
                    logger.info(f"DEBUG: Listing contents of directory {debug_dir}: {os.listdir(debug_dir)}")
                except Exception as e:
                    logger.error(f"DEBUG: Could not list directory {debug_dir}. Error: {e}")
