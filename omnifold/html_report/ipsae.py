...
def parse_cif_atom_line(line,fielddict):
    ...
    b_factor =        linelist[ fielddict['B_iso_or_equiv'] ]
    ...
    for line in PDB:
        ...
        if line.startswith("ATOM") or line.startswith("HETATM"):
            ...
            if atom['atom_name'] == "CA" or "C1" in atom['atom_name']:
                token_mask.append(1)
                plddt_from_cif.append(atom['b_factor'])
                residues.append({
            ...
            if atom['atom_name'] == "CB" or "C3" in atom['atom_name'] or (atom['residue_name']=="GLY" and atom['atom_name']=="CA"):
                cb_plddt_from_cif.append(atom['b_factor'])
                cb_residues.append({
                    'atom_num': atom['atom_num'],
                    'coor': np.array([atom['x'], atom['y'], atom['z']]),
                    'res': atom['residue_name'],
...
if chai1:
    ...
    if os.path.exists(pae_file_path):
        ...
        pae_matrix_chai1 = np.array(data_pae['pae'])
        pae_matrix = pae_matrix_chai1[np.ix_(token_array.astype(bool), token_array.astype(bool))]
    else:
        print("Chai1 PAE file does not exist: ", pae_file_path)
        sys.exit()
    
    plddt = np.array(plddt_from_cif)
    cb_plddt = np.array(cb_plddt_from_cif)

    scores_file_path = pae_file_path.replace("pae", "scores")
    iptm_chai1 = {chain1: {chain2: 0 for chain2 in unique_chains if chain1 != chain2} for chain1 in unique_chains}
    if os.path.exists(scores_file_path):
        scores_data = np.load(scores_file_path)
        if 'per_chain_pair_iptm' in scores_data:
            chai_iptm_matrix = scores_data['per_chain_pair_iptm']
            if chai_iptm_matrix.ndim == 3: # Handle batched dimension
                chai_iptm_matrix = chai_iptm_matrix[0]
            chain_map = {chain_id: i for i, chain_id in enumerate(unique_chains)}
            for c1 in unique_chains:
                for c2 in unique_chains:
                    if c1 == c2: continue
                    idx1 = chain_map[c1]
                    idx2 = chain_map[c2]
                    iptm_chai1[c1][c2] = chai_iptm_matrix[idx1, idx2]
        else:
            print("Chai1 scores file does not contain 'per_chain_pair_iptm': ", scores_file_path)
    else:
        print("Chai1 scores file does not exist: ", scores_file_path)
...
for pair in sorted(chainpairs):
    ...
    for pair in (pair1, pair2):
        ...
        if chai1: iptm_af=iptm_chai1[chain1][chain2]
        ...
        if chain1 > chain2:
            ...
            if chai1:
                iptm_af_value=max(iptm_chai1[chain1][chain2], iptm_chai1[chain2][chain1])
