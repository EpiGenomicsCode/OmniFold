...
class ConfigGenerator:
    ...
    def generate_configs(self, job_input: JobInput, output_dir: Path, cli_config: dict) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            # Priority: If MSAs were generated by our pipeline (indicated by protein_id_to_a3m_path being populated),
            # we MUST generate a Boltz config that uses these MSA paths.
            if job_input.protein_id_to_a3m_path and any(s.molecule_type == 'protein' for s in job_input.sequences):
                ...
                if boltz_config_path:
                    ...
                    # This is a critical failure if we generated MSAs but can't make a config for them.
                ...
                # User provided Boltz YAML, and we did NOT generate MSAs for it in this run (or no proteins to add MSAs for).
                # We assume this YAML is self-contained for Boltz (e.g., has its own MSA paths, uses 'empty', or user intends to use --use_msa_server if running manually).
            ...
            elif any(s.molecule_type != 'unknown' for s in job_input.sequences): # No original Boltz config, but sequences exist for Boltz.
    ...
    def _generate_boltz_yaml_from_job_input(
        ...
        for seq_info in job_input.sequences:
            ...
            if seq_info.molecule_type == "protein":
                # Construct MSA path relative to the job_output_root inside container
                container_job_output_root = "/data/job_output" # Must match runner.py
                msa_filename_only = f"msa_{chain_id}.a3m"
                # The msa_path_for_chain might be an absolute host path or just a filename if not found yet
                # For the generated YAML, we want a path that Boltz can find *inside* the container
                # This assumes msa_A.a3m etc. are in job_output_root/msa_generation/
                ...
                entity = {
                    "protein": {
                        "id": chain_id,
                        "sequence": seq_info.sequence,
                        "msa": container_msa_path # Use container-absolute path
                    }
                }
                # Handle 'msa: empty' if msa_path_for_chain indicates a singleton or is explicitly 'empty'
                # is_a3m_singleton expects an actual file path to check content
                ...
                if msa_path_for_chain: # if a host path was determined
                    if Path(msa_path_for_chain).name == "empty" or \
                     ...
                     entity["protein"]["msa"] = "empty" # Default to empty if no MSA is available
            elif seq_info.molecule_type == "rna":
