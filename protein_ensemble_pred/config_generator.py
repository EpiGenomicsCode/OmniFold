    ...
    def generate_configs(self, job_input: JobInput, output_dir: Path, cli_config: dict) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            if job_input.original_af3_config_path and job_input.has_msa:
                # User provided AF3 JSON and InputHandler confirmed it has MSAs.
                # job_input.af3_data_json should also point to this original path.
                ...
                # This means MSAs were generated by AF3 MSA pipeline, 
                # or InputHandler identified the input AF3 JSON as a _data.json (even if original_af3_config_path is set, has_msa was false, so we used its derivative).
                config_paths["af3_config_path"] = job_input.af3_data_json
                logger.info(f"Using AF3 data JSON (from MSA step or identified input) for AF3 inference: {config_paths['af3_config_path']}")
            elif not job_input.is_boltz_config: # Only generate if not a Boltz-specific input that shouldn't run AF3
                logger.info("Attempting to generate new AlphaFold3 JSON config for inference.")
                # Suffix for generated file, helps distinguish from MSA generation inputs
                af3_inference_filename = f"{job_input.name_stem}_af3_inference_generated.json"
                ...
                if af3_config_path:
                    ...
                    logger.warning("Failed to generate AlphaFold3 JSON config for inference.")
            else:
